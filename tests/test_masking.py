...
def test_lens_to_mask(t: Tensor) -> None:
	...
	lengths_vector = torch.tensor([base_length, base_length + 2, base_length + 3, base_length + 5], dtype=torch.long)
...
def test_and_masks(sequence_tensors: list[Tensor], empty_optional_tensor_sequence: list[Tensor | None]) -> None:
	list_tensors = sequence_tensors
...
def test_or_masks(sequence_tensors: list[Tensor], empty_optional_tensor_sequence: list[Tensor | None]) -> None:
	list_tensors = sequence_tensors
