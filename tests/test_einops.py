...
def test_pack_with_inverse_round_trip_for_tensor_and_sequence(pack_input: Tensor | list[Tensor], pattern: str, tensor_shift: float) -> None:
	if torch.is_tensor(pack_input):
		...
		assert torch.is_tensor(round_trip), 'pack_with_inverse round-trip output is not a tensor for tensor input.'
		...
		assert torch.is_tensor(shifted_round_trip), 'pack_with_inverse shifted round-trip output is not a tensor for tensor input.'
	...
	assert isinstance(pack_input, list), 'pack_with_inverse list test input must be a list of tensors.'
	packed, inverse = pack_with_inverse(pack_input, pattern)
	round_trip = inverse(packed, None)
	shifted_round_trip = inverse(packed + tensor_shift, None)

	assert isinstance(round_trip, list), 'pack_with_inverse round-trip output is not a list for sequence input.'
	assert isinstance(shifted_round_trip, list), 'pack_with_inverse shifted output is not a list for sequence input.'
...
def test_pack_one_returns_packed_tensor_with_shape_metadata(t: Tensor, einops_pack_one_pattern: str) -> None:
	...
	assert len(packedShape) == 1, (
		f'pack_one packed_shape has {len(packedShape)} entries, expected 1 for single-tensor input with shape {tuple(t.shape)}.'
	)

def test_unpack_one_round_trip_restores_original_tensor(
	t: Tensor, pack_one_result: tuple[Tensor, Sequence[tuple[int, ...] | list[int]]], einops_pack_one_pattern: str
) -> None:
	packed, packedShape = pack_one_result
	result = unpack_one(packed, packedShape, einops_pack_one_pattern)

	assert result.shape == t.shape, (
		f'unpack_one returned shape {tuple(result.shape)}, expected {tuple(t.shape)} for {einops_pack_one_pattern=}.'
	)
	assert torch.equal(result, t), f'unpack_one round-trip mismatch for input shape {tuple(t.shape)} and {einops_pack_one_pattern=}.'
