...
def _fill_value_for_tensor(tensor_value: Tensor, *, float_fill_value: float, int_fill_value: int) -> float | int:
...
def _assert_right_padded_sequence_matches_inputs(
	list_tensors: list[Tensor], padded_tensors: list[Tensor], *, max_last_dimension_length: int, fill_value: float
) -> None:
...
@pytest.mark.parametrize(
	('left_pad_width', 'right_pad_width'), [pytest.param(LEFT_PAD_WIDTH, RIGHT_PAD_WIDTH, id='asymmetric-padding-widths')]
)
def test_pad_at_dim(t: Tensor, left_pad_width: int, right_pad_width: int) -> None:
	fill_value = _fill_value_for_tensor(t, float_fill_value=PAD_FILL_FLOAT_73, int_fill_value=PAD_FILL_INT_73)
	...
	for dimension_index in _dimension_inputs_for_tensor(t):
		...
		result = pad_at_dim(t, (left_pad_width, right_pad_width), dim=dimension_index, value=float(fill_value))
		...
		right_padding_slice = result.narrow(
			normalized_dimension_index, result.shape[normalized_dimension_index] - right_pad_width, right_pad_width
		)
...
def test_pad_left_at_dim(t: Tensor, left_pad_width: int) -> None:
	fill_value = _fill_value_for_tensor(t, float_fill_value=PAD_FILL_FLOAT_79, int_fill_value=PAD_FILL_INT_79)

	for dimension_index in _dimension_inputs_for_tensor(t):
		expected = pad_at_dim(t, (left_pad_width, 0), dim=dimension_index, value=float(fill_value))
		result = pad_left_at_dim(t, left_pad_width, dim=dimension_index, value=float(fill_value))
...
def test_pad_right_at_dim(t: Tensor, right_pad_width: int) -> None:
	fill_value = _fill_value_for_tensor(t, float_fill_value=PAD_FILL_FLOAT_83, int_fill_value=PAD_FILL_INT_83)

	for dimension_index in _dimension_inputs_for_tensor(t):
		expected = pad_at_dim(t, (0, right_pad_width), dim=dimension_index, value=float(fill_value))
		result = pad_right_at_dim(t, right_pad_width, dim=dimension_index, value=float(fill_value))
...
def test_pad_left_at_dim_to(t: Tensor, length_step: int) -> None:
	fill_value = _fill_value_for_tensor(t, float_fill_value=PAD_FILL_FLOAT_89, int_fill_value=PAD_FILL_INT_89)
	...
	for dimension_index in _dimension_inputs_for_tensor(t):
		...
		for target_length in target_lengths:
			result = pad_left_at_dim_to(t, target_length, dim=dimension_index, value=float(fill_value))
...
@pytest.mark.parametrize('length_step', [pytest.param(LENGTH_STEP, id='length-step-three')])
def test_pad_right_at_dim_to(t: Tensor, length_step: int) -> None:
	fill_value = _fill_value_for_tensor(t, float_fill_value=PAD_FILL_FLOAT_97, int_fill_value=PAD_FILL_INT_97)
	...
	for dimension_index in _dimension_inputs_for_tensor(t):
		...
		for target_length in target_lengths:
			result = pad_right_at_dim_to(t, target_length, dim=dimension_index, value=float(fill_value))
...
def test_pad_sequence(sequence_tensors: list[Tensor], empty_tensor_sequence: list[Tensor]) -> None:
	...
	right_padded_output = pad_sequence(list_tensors, dim=-1, value=PAD_FILL_FLOAT_101, left=False, return_stacked=False, return_lens=False)
	...
	_assert_right_padded_sequence_matches_inputs(
		list_tensors, right_padded_output, max_last_dimension_length=max_last_dimension_length, fill_value=PAD_FILL_FLOAT_101
	)

	left_padded_output = pad_sequence(list_tensors, dim=-1, value=PAD_FILL_FLOAT_101, left=True, return_stacked=False, return_lens=False)
	...
	_assert_left_padded_sequence_matches_inputs(
		list_tensors, left_padded_output, max_last_dimension_length=max_last_dimension_length, fill_value=PAD_FILL_FLOAT_101
	)

	lengths_output = pad_sequence(
		list_tensors, dim=-1, value=PAD_FILL_FLOAT_101, left=False, return_stacked=False, return_lens=True, pad_lens=False
	)
	...
	pad_lengths_output = pad_sequence(
		list_tensors, dim=-1, value=PAD_FILL_FLOAT_101, left=False, return_stacked=False, return_lens=True, pad_lens=True
	)
	...
	stacked_two_dimensional_output = pad_sequence(
		row_two_tensors, dim=-1, value=PAD_FILL_FLOAT_101, left=False, return_stacked=True, return_lens=True
	)
	...
	assert tuple(stacked_two_dimensional_tensors.shape) == (len(row_two_tensors), ROW_COUNT_TWO, max_two_dimensional_length), (
		'pad_sequence returned incorrect stacked two-dimensional shape.'
	)

	expected_stacked_lengths = torch.tensor([tensor_value.shape[-1] for tensor_value in row_two_tensors], device=stacked_lengths.device)
	...
	stacked_dim_zero_output = pad_sequence(
		width_three_tensors, dim=0, value=PAD_FILL_FLOAT_101, left=False, return_stacked=True, return_lens=True
	)
	...
	assert tuple(stacked_dim_zero_tensors.shape) == (len(width_three_tensors), max_dim_zero_length, WIDTH_THREE), (
		'pad_sequence returned incorrect dim=0 stacked shape.'
	)

	expected_dim_zero_lengths = torch.tensor(
		[tensor_value.shape[0] for tensor_value in width_three_tensors], device=stacked_dim_zero_lengths.device
	)
	assert torch.equal(stacked_dim_zero_lengths, expected_dim_zero_lengths), 'pad_sequence returned incorrect dim=0 lengths.'

def test_pad_sequence_and_cat(sequence_tensors: list[Tensor], empty_tensor_sequence: list[Tensor]) -> None:
	list_tensors = sequence_tensors
	...
	manual_right_output = pad_sequence(
		one_dimensional_float_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=False, return_stacked=False, return_lens=False
	)
	...
	right_result = pad_sequence_and_cat(one_dimensional_float_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=False, dim_cat=0)
	...
	manual_left_output = pad_sequence(
		one_dimensional_float_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=True, return_stacked=False, return_lens=False
	)
	...
	left_result = pad_sequence_and_cat(one_dimensional_float_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=True, dim_cat=0)
	...
	manual_two_dimensional_output = pad_sequence(
		row_two_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=False, return_stacked=False, return_lens=False
	)
	...
	two_dimensional_result = pad_sequence_and_cat(row_two_tensors, dim=-1, value=PAD_FILL_FLOAT_103, left=False, dim_cat=0)
	...
	manual_dim_zero_output = pad_sequence(
		width_three_tensors, dim=0, value=PAD_FILL_FLOAT_103, left=False, return_stacked=False, return_lens=False
	)
	...
	dim_zero_result = pad_sequence_and_cat(width_three_tensors, dim=0, value=PAD_FILL_FLOAT_103, left=False, dim_cat=-1)
