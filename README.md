...
| `pack_with_inverse(t, pattern)` | Wraps `einops.pack` and stores the corresponding inverse. Input `t` may be one tensor or a `list[Tensor]`. When `t` is one tensor, the inverse has type `InversePackTensor` and returns one tensor. When `t` is a `list[Tensor]`, the inverse has type `InversePackListTensors` and returns `list[Tensor]`. The inverse optionally accepts a different `inv_pattern` for unpacking. |
